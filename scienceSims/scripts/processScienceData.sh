...
usage() {
    ...
    echo "" 1>&2
    echo "Usage: $0 [-d DATADIR] [-r <RERUN>] [-c CORES] [-n] [-w WORKDIR]" 1>&2
    echo "" 1>&2
    echo "    -d <DATADIR> : path to raw data" 1>&2
    ...
    echo "    -w WORKDIR : directory to use for work (default: ${WORKDIR})" 1>&2
    echo "" 1>&2
    exit 1
}

while getopts "c:d:Dnr:w:" opt; do
    case "${opt}" in
        c)
            CORES=${OPTARG}
            ;;
